...
I then searched for a solution to this problem and found this answer: https://www.tidyverse.org/blog/2020/04/dplyr-1-0-0-and-vctrs/

I therefore added the mutute to change Age to a character variable. However, another error popped up:
>Error in mutate(Age = as.character(Age)) : object 'Age' not found

I realised that this has to do with the fact that the dataset contain 'Age' is not loaded into the function. I therefore tried to insert the data in the function so that it can pick the Age variable up. However, this still did not work as I received the following error message:
